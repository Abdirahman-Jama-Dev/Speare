...
export class ExecutorRegistry {
  ...
  register(executor: StepExecutor): void {
    if (this.executors.has(executor.type)) {
      throw new FrameworkError(`Executor already registered for step type: "${executor.type}"`);
    }
  ...
  async execute(step: AnyStep, context: ExecutionContext): Promise<ExecutionContext> {
    ...
    if (!executor) throw new FrameworkError(`No executor found for step type: "${type}"`);
